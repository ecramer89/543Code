...
int MAX_TIMES_SAME_CHANGES=6;
int THRESHOLD_TIMES_CHANGE_UNDER_CONSTANT_MOVEMENT=3;
...
void changeDisplay() {
  int param_to_change;
  while (acceptableToChangeCurrentMove(param_to_change) || acceptableToChangeSameParameter(param_to_change)) {
  ...
  if (param_to_change == CURR_MOVEMENT_IDX) {
    changes_under_current_movement=0;
  } else {
    changes_under_current_movement++;
  }
