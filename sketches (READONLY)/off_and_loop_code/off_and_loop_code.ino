...
void disconnectCurrentAnode(int index_of_tentacle) {
  int anodePin = pins[index_of_tentacle][curr_pattern_component[index_of_tentacle][INDEX_OF_ANODE_PIN]];
  disconnect(anodePin);
}


void disconnectCurrentCathode(int index_of_tentacle) {
  int groundPin = pins[index_of_tentacle][curr_pattern_component[index_of_tentacle][INDEX_OF_CATHODE_PIN]];
...
void groundCathodeOfCurrentPatternComponent(int index_of_tentacle) {
  int ground_pin = pins[index_of_tentacle][curr_pattern_component[index_of_tentacle][INDEX_OF_CATHODE_PIN]];
  makeGround(ground_pin);
}


void lightupAnodeOfCurrentPatternCathode(int index_of_tentacle) {
  int anode_pin = pins[index_of_tentacle][curr_pattern_component[index_of_tentacle][INDEX_OF_ANODE_PIN]];
  lightUp(anode_pin);
}
...
void setIndexOfNextPatternComponent(int index_of_tentacle) {
  if (index_of_current_pattern_component[index_of_tentacle] < pattern_length) {
    index_of_current_pattern_component[index_of_tentacle]++;
  }
  else {
