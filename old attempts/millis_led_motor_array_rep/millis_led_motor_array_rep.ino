...
bool lightSwitchTimeHasElapsed(int currMillis) {
  return abs(currMillis - prevMillis) >= COLOR_SWITCH_TIME;
...
void handleTentacles() {
  ...
  //cycle through all colourst
  int r = curr_led_arr[R];
  int g = curr_led_arr[G];
  int b = curr_led_arr[B];
  long currentMillis = millis();

  if (lightSwitchTimeHasElapsed(currentMillis) && color_counter == 0) {
  ...
  if (lightSwitchTimeHasElapsed(currentMillis) && color_counter == 1) {
    pinMode(r, INPUT);
    pinMode(g, OUTPUT);
    digitalWrite(g, HIGH);
    prevMillis = millis();
  ...
  if (lightSwitchTimeHasElapsed(currentMillis) && color_counter == 2) {
    pinMode(g, INPUT);
    pinMode(b, OUTPUT);
    digitalWrite(b, HIGH);
    prevMillis = millis();
  ...
  if (lightSwitchTimeHasElapsed(currentMillis) && color_counter == 3) {
    pinMode(b, OUTPUT);
